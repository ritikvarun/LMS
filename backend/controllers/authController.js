...
export const signUp=async (req,res)=>{
    ...
    try {
        ...
        // Security check: Only grant 'educator' if email matches ADMIN_EMAIL or secret matches ADMIN_SECRET_KEY
        ...
        if (adminEmail && email.toLowerCase().trim() === adminEmail) {
            ...
            if (adminSecretKey && adminSecret && adminSecret.trim() === adminSecretKey) {
                assignedRole = "educator"
            } else {
                return res.status(403).json({ message: "Invalid Admin Secret Key! Only authorized instructors can register as Educator." })
            }
        }
...
export const googleSignup = async (req,res) => {
    try {
        ...
        if(!user){
            // Automatically assign educator role if matching ADMIN_EMAIL, otherwise default to student
            const assignedRole = (adminEmail && email.toLowerCase().trim() === adminEmail) ? "educator" : "student"
